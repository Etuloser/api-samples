...
def set_banner(banner_url):
    channels_response = (
        youtube.channels().list(mine=True, part="brandingSettings").execute()
    )

    if "brandingSettings" not in channels_response["items"][0]:
        channels_response["items"][0]["brandingSettings"]["image"][
            "bannerExternalUrl"
        ] = []
    ...
    banner_mobile_url = channels_update_response["brandingSettings"]["image"][
...
if __name__ == "__main__":
    argparser.add_argument("--file", required=True, help="Path to banner image file.")
