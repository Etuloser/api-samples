...
def get_comments(youtube, parent_id):
    results = (
        youtube.comments()
        .list(part="snippet", parentId=parent_id, textFormat="plainText")
        .execute()
    )
...
def insert_comment(youtube, parent_id, text):
    insert_result = (
        ...
        .insert(
            part="snippet",
            body=dict(snippet=dict(parentId=parent_id, textOriginal=text)),
        )
        .execute()
    )
...
def update_comment(youtube, comment):
    ...
    update_result = youtube.comments().update(part="snippet", body=comment).execute()
...
def set_moderation_status(youtube, comment):
    youtube.comments().setModerationStatus(
        id=comment["id"], moderationStatus="published"
    ).execute()
...
def mark_as_spam(youtube, comment):
    youtube.comments().markAsSpam(id=comment["id"]).execute()
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--text", help="Required; text that will be used as comment."
    )
