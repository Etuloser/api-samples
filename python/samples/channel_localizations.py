...
def set_channel_localization(youtube, args):
    results = (
        youtube.channels().list(part="localizations", id=args.channel_id).execute()
    )
    ...
    if args.default_language:
        results = (
            youtube.channels()
            .list(part="brandingSettings", id=args.channel_id)
            .execute()
        )
        ...
        branding_settings_channel["brandingSettings"]["channel"][
            "defaultLanguage"
        ] = args.default_language
        language_result = (
            youtube.channels()
            .update(part="brandingSettings", body=branding_settings_channel)
            .execute()
        )
        updated_default_language = language_result["brandingSettings"]["channel"][
            "defaultLanguage"
        ]
        print("Updated language to %s" % updated_default_language)

    update_result = (
        youtube.channels().update(part="localizations", body=channel).execute()
    )
    ...
    print(
        (
            "Updated channel '%s' localized title and description to "
...
def get_channel_localization(youtube, channel_id, language):
    results = (
        youtube.channels().list(part="snippet", id=channel_id, hl=language).execute()
    )
    ...
    print(
        "Channel description is '%s' in language '%s'"
...
def list_channel_localizations(youtube, channel_id):
    results = (
        youtube.channels().list(part="snippet,localizations", id=channel_id).execute()
    )
