...
def create_reporting_job(youtube_reporting, report_type_id, **kwargs):
    ...
    reporting_job = (
        ...
        .create(body=dict(reportTypeId=args.report_type, name=args.name), **kwargs)
        .execute()
    )
    ...
    try:
        ...
        if args.report_type:
            create_reporting_job(
                youtube_reporting, args, onBehalfOfContentOwner=args.content_owner
            )
