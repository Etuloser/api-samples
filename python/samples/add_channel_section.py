...
def enable_browse_view(youtube):
    channels_list_response = (
        youtube.channels().list(part="brandingSettings", mine=True).execute()
    )
...
def add_channel_section(youtube, args):
    ...
    body = dict(
        snippet=dict(
            type=args.type, style=args.style, title=args.title, position=args.position
        ),
        contentDetails=dict(channels=channels, playlists=playlists),
    )

    youtube.channelSections().insert(part="snippet,contentDetails", body=body).execute()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--playlists", help="One or more playlist ids, comma-separated (e.g. PL...)."
    )
    parser.add_argument(
        "--channels", help="One or more channel ids, comma-separated (e.g. UC...)."
    )
