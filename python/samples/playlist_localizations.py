...
def set_playlist_localization(youtube, args):
    results = (
        youtube.playlists()
        .list(part="snippet,localizations", id=args.playlist_id)
        .execute()
    )
    ...
    if args.default_language:
        ...
        if args.default_language in playlist["localizations"]:
            ...
            playlist["snippet"]["description"] = playlist["localizations"][
                args.default_language
            ]["description"]
    ...
    update_result = (
        youtube.playlists()
        .update(part="snippet,localizations", body=playlist)
        .execute()
    )
    ...
    if args.language:
        for language in update_result["localizations"]:
            ...
            if language.lower() == args.language.lower():
                ...
                print(
                    (
                        "Updated playlist '%s' localized title to '%s'"
...
def get_playlist_localization(youtube, args):
    results = (
        youtube.playlists()
        .list(part="snippet", id=args.playlist_id, hl=args.language)
        .execute()
    )
    ...
    print(
        (
            'Playlist title is "%s" and description is "%s" in language "%s"'
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--default_language", help="Default language to set for the playlist."
    )
