...
def set_video_localization(youtube, args):
    ...
    results = (
        youtube.videos().list(part="snippet,localizations", id=args.video_id).execute()
    )
    ...
    if args.default_language and args.default_language in video["localizations"]:
        ...
        video["snippet"]["title"] = video["localizations"][args.default_language][
            "title"
        ]
        video["snippet"]["description"] = video["localizations"][args.default_language][
            "description"
        ]

    # Update the video resource.
    update_result = (
        youtube.videos().update(part="snippet,localizations", body=video).execute()
    )
    ...
    if args.language:
        for language in update_result["localizations"]:
            ...
            if language.lower() == args.language.lower():
                ...
                print(
                    (
                        "Updated video '%s' localized title to '%s'"
...
def get_video_localization(youtube, args):
    results = (
        youtube.videos()
        .list(part="snippet", id=args.video_id, hl=args.language)
        .execute()
    )
    ...
    print(
        (
            "Video title is '%s' and description is '%s' in language '%s'"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--default_language", help="Default language of the video to update."
    )
