...
def set_channel_section_localization(youtube, args):
    ...
    results = (
        ...
        .list(part="snippet,contentDetails,localizations", id=args.channel_section_id)
        .execute()
    )
    ...
    if args.default_language:
        ...
        if args.default_language in channel_section["localizations"]:
            channel_section["snippet"]["title"] = channel_section["localizations"][
                args.default_language
            ]["title"]

    update_result = (
        youtube.channelSections()
        .update(part="snippet,contentDetails,localizations", body=channel_section)
        .execute()
    )
    ...
    if args.language:
        for language in update_result["localizations"]:
            ...
            if language.lower() == args.language.lower():
                ...
                print(
                    (
                        "Updated channel section %s localized title to %s in '%s'."
                        % (
                            args.channel_section_id,
                            localization["title"],
                            args.language,
                        )
                    )
    ...
    if (
        args.default_language
        and args.default_language == update_result["snippet"]["defaultLanguage"]
    ):
...
def get_channel_section_localization(youtube, args):
    results = (
        youtube.channelSections()
        .list(part="snippet", id=args.channel_section_id, hl=args.language)
        .execute()
    )
    ...
    print(
        (
            "The channel section's title is '%s' in language '%s'."
...
def list_channel_section_localizations(youtube, args):
    results = (
        youtube.channelSections()
        .list(part="snippet,localizations", id=args.channel_section_id)
        .execute()
    )
    ...
    if "localizations" in results["items"][0]:
        ...
        for language, localization in localizations.items():
            print(
                (
                    "The channel section title is '%s' in language '%s'."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--default_language", help="Default language of the channel section to update."
    )
    ...
    if not args.channel_section_id and not args.mine:
        exit(
            "You must either specify a channel section ID using the "
