...
def upload_caption(youtube, video_id, language, name, file):
    insert_result = (
        ...
        .insert(
            ...
            body=dict(
                snippet=dict(
                    videoId=video_id, language=language, name=name, isDraft=True
                )
            ),
            media_body=file,
        )
    ...
    print(
        (
            "Uploaded caption track '%s(%s) in '%s' language, '%s' status."
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--name", help="Caption track name", default="YouTube for Developers"
    )
    ...
    argparser.add_argument(
        "--captionid", help="Required; ID of the caption track to be processed"
    )
